...
public:
	Solver_MCSVM_CS(const problem *prob, int nr_class, double *C, double eps =
			0.1, int max_iter = 100000);
	~Solver_MCSVM_CS();
	void Solve(double *w);
private:
	...
	while (iter < max_iter) {
		...
		for (s = 0; s < active_size; s++) {
			...
			if (Ai > 0) {
				...
				if (y_index[i] < active_size_i[i])
					if (alpha_i[(int) prob->y[i]] < C[GETI(i)]
							&& G[y_index[i]] < minG)
						minG = G[y_index[i]];

				for (m = 0; m < active_size_i[i]; m++) {
					if (be_shrunk(i, m, y_index[i], alpha_i[alpha_index_i[m]],
							minG)) {
						active_size_i[i]--;
						while (active_size_i[i] > m) {
							if (!be_shrunk(i, active_size_i[i], y_index[i],
									alpha_i[alpha_index_i[active_size_i[i]]],
									minG)) {
								swap(alpha_index_i[m],
...
static void solve_l2r_l1l2_svr(const problem *prob, double *w,
	...
	while (iter < max_iter) {
		...
		for (s = 0; s < active_size; s++) {
			...
			beta[i] = min(max(beta[i] + d, -upper_bound[GETI(i)]),
					upper_bound[GETI(i)]);
			d = beta[i] - beta_old;
...
void L1RLRSolver::Init(const problem *prob, double eps_) {
	prob_col = prob;

	l = prob_col->l;
	w_size = prob_col->n;

	index = new int[w_size];
	y = new schar[l];
	Hdiag = new double[w_size];
	Grad = new double[w_size];
	wpd = new double[w_size];
	xjneg_sum = new double[w_size];
	xTd = new double[l];
	exp_wTx = new double[l];
	exp_wTx_new = new double[l];
	tau = new double[l];
	D = new double[l];
	w = new double[w_size];
	...
	eps = eps_ * double(std::max(std::min(pos, neg), 1)) / double(l);
}

void L1RLRSolver::Free() {
	delete[] index;
	delete[] y;
	delete[] Hdiag;
	delete[] Grad;
...
void L1RLRSolver::Solve(const double *C_vals, const int C_nr, double **w_sol) {
	for (int i = 0; i != l; ++i) {
		if (prob_col->y[i] > 0.5) {
			y[i] = 1;
		} else {
	...
	// Initial w can be set here.
	std::fill(w, w + w_size, 0);

	for (int C_i = 0; C_i != C_nr; ++C_i) {
		...
		double C[3] = { C_vals[C_i], 0, C_vals[C_i] };

		std::fill(exp_wTx, exp_wTx + l, 0);
		...
		std::copy(w, w + w_size, w_sol[C_i]);
	}
}

void solve_l1r_lr_range_C(const problem *prob_col, const double *C_vals,
		const int C_nr, const double eps_, double **w_sol) {
	int l = prob_col->l;
	int w_size = prob_col->n;
...
void solve_l1r_lr(const problem *prob_col, double *w, double eps, double Cp,
	...
	if (w_init == NULL) {
		...
		std::copy(w_init, w_init + w_size, w);
	}
...
static void train_one(const problem *prob, const parameter *param, double *w,
		double Cp, double Cn, const double *w_init) {
	double eps = param->eps;
	int pos = 0;
	int neg = 0;
	...
	case L1R_LR: {
		...
		feature_node *x_space = NULL;
		transpose(prob, &x_space, &prob_col);
		solve_l1r_lr(&prob_col, w, primal_solver_tol, Cp, Cn, w_init);
		delete[] prob_col.y;
		delete[] prob_col.x;
		delete[] x_space;
		break;
...
model* train(const problem *prob, const parameter *param,
	...
	if (check_regression_model(model_)) {
		...
		model_->nr_class = 2;
		model_->label = NULL;
		train_one(prob, param, &model_->w[0], 0, 0, w_init);
	} else {
		int nr_class;
		int *label = NULL;
		int *start = NULL;
		...
		if (param->solver_type == MCSVM_CS) {
			...
			if (nr_class == 2) {
				...
				train_one(&sub_prob, param, &model_->w[0], weighted_C[0],
						weighted_C[1], w_init);
			} else {
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold,
		double *target, const double *w_init) {
	int i;
	int *fold_start;
	int l = prob->l;
	int *perm = Malloc(int, l);
	...
	for (i = 0; i < nr_fold; i++) {
		...
		struct model *submodel = train(&subprob, param, w_init);
		for (j = begin; j < end; j++)
...
static const char *solver_type_table[] = { "L2R_LR", "L2R_L2LOSS_SVC_DUAL",
		"L2R_L2LOSS_SVC", "L2R_L1LOSS_SVC_DUAL", "MCSVM_CS", "L1R_L2LOSS_SVC",
		"L1R_LR", "L2R_LR_DUAL", "", "", "", "L2R_L2LOSS_SVR",
		"L2R_L2LOSS_SVR_DUAL", "L2R_L1LOSS_SVR_DUAL", NULL };
